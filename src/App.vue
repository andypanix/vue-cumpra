  ...
  <section id="app">
    ...
    <div class="query">
      <div class="wrapper" v-if="micro == false">
        ...
        <input aria-label="Ask me something" autocomplete="off" v-model="query"
               class="queryform" @keyup.enter="submit()"
               placeholder="Ask me something..." autofocus type="text">
        <i class="material-icons iicon t2s" @click="mute(true)"
           v-if="muted == false">volume_up</i>
        <i class="material-icons iicon t2s" @click="mute(false)" v-else>volume_off</i>
      </div>
      <div class="wrapper" v-else>
        <i class="material-icons iicon recording"
           @click="microphone(false)">mic</i><input class="queryform"
                                                    :placeholder="speech"
                                                    readonly>
      </div>
    </div>
    ...
    <main class="wrapper ai-window">
      ...
      <div v-if="answers.length == 0 && online == true">
        <h1 class="title mdc-typography--headline">
          ...
          <br>
          <br>
          Benvenuto in VueCumprà, chiedimi qualcosa!

          <p class="mdc-typography--body2">Prova a scrivere "ciao"</p>
        </h1>
      </div>
      ...
      <div v-if="answers.length == 0 && online == false">
        <h1 class="title mdc-typography--headline">
          ...
          <p class="mdc-typography--body2">It looks like you are not connected
            to the internet, this webpage <b>requires</b> internet connection,
            to process your requests</p>
        </h1>
      </div>
          ...
          <td>
            ...
            <div v-for="r in a.result.fulfillment.messages">
              ...
              <div class="mdc-card" v-if="r.type == 'basic_card'">
                <img :title="r.image.accessibilityText"
                     :alt="r.image.accessibilityText"
                     class="mdc-card__media-item" :src="r.image.url"
                     v-if="r.image">
                ...
                <section class="mdc-card__actions" v-for="button in r.buttons">
                  <a
                    class="mdc-button mdc-button--compact mdc-button--primary mdc-card__action"
                    target="_blank" :href="button.openUrlAction.url">{{button.title}}
                    <i class="material-icons openlink">open_in_new</i></a>
                </section>
              </div>
                ...
                <carousel
                  ...
                  <slide v-for="item in r.items" :key="item.id">
                    <div class="mdc-card slide">
                      ...
                      <section class="mdc-card__primary">
                        <h1
                          class="mdc-card__title mdc-card__title mdc-theme--primary"
              ...
              <div class="mdc-list-group mdc-card" v-if="r.type == 'list_card'">
                ...
                <ul class="mdc-list mdc-list--two-line mdc-list--avatar-list"
                  ...
                  <li class="mdc-list-item">
                    <img :title="item.image.accessibilityText"
                         :alt="item.image.accessibilityText"
                         class="mdc-list-item__start-detail" width="56"
                         height="56" :src="item.image.url" v-if="item.image"/>
                    <span class="mdc-list-item__text">
                      {{item.title}}<span class="mdc-list-item__text__secondary">{{item.description}}</span>
                    </span>
                  </li>
                </ul>
              </div>
              ...
              <div v-if="r.type == 'suggestion_chips'" class="chips">
                <div class="suggestion" @click="autosubmit(s.title)"
